#!/bin/sh
# $Id: sync-distfiles-finkinfo.sh,v 1.14 2014/03/27 21:45:57 gecko2 Exp $
...
###
GENERATEDIR="/home/fink/finkinfo"
HTTPSNAPDIR="/home/fink/httpsnap"
LOGDIR="/home/fink/log"
SCRIPTDIR="/home/fink/scripts"
...
${SCRIPTDIR}/generate-distfiles-and-finkinfo-mirror.pl #--skip-update
test -s ${HTTPSNAPDIR}/FORCED && mv ${HTTPSNAPDIR}/FORCED ${HTTPSNAPDIR}/FORCED.old
...
if [ \! -s ${TMPDIR}/mirror.lock ]; then
	...
	if [ -s ${HTTPSNAPDIR}/FORCED ];then
		...
		echo "10.4/unstable/crypto/foo.info" >>${LOGDIR}/change.log
		echo "10.7/stable/main/foo.info" >>${LOGDIR}/change.log
		echo "10.7/stable/crypto/foo.info" >>${LOGDIR}/change.log
		echo "10.9-libcxx/stable/main/foo.info" >>${LOGDIR}/change.log
		echo "10.9-libcxx/stable/crypto/foo.info" >>${LOGDIR}/change.log
		rm -f ${TMPDIR}/FORCE
	fi
	if [ "$(grep -e '.info$' -e '.patch$' "${LOGDIR}"/change.log)" != "" ]; then
		...
		for osx in 10.4 10.7 10.9-libcxx; do
			...
			for release in stable unstable; do
				for tree in main crypto; do
					if [ "$(grep "${osx}"/"${release}"/"${tree}" "${LOGDIR}"/change.log)" != "" ]; then
						...
						elif [ "${osx}" = "10.9-libcxx" ]; then
							for dist in 10.10 10.11; do
								printf " Creating symlinks and timestamps for "${dist}"-"${release}"-"${tree}"\n"
								ln -sf ${osx}-${release}-${tree}.tbz ${HTTPSNAPDIR}/${dist}-${release}-${tree}.tbz
								for check in md5 sha1 sha256; do
									ln -sf ${osx}-${release}-${tree}.tbz.${check} ${HTTPSNAPDIR}/${dist}-${release}-${tree}.tbz.${check}
